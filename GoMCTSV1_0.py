...
class MCTS():
    ...
    def getBestMove(self,board,width,player):
        ...
        with Pool(processes = poolCount) as pool: 
            for score,move in pool.map_async(self.search,roots).get():
                #print(score)
                child = self.__tree.addChild(move)
...
class node():
    ...
    def getBestMove(self):
        best = -9999999999
        #print(f"get best move. children: {self.__children}")
        for child in self.__children:
            #print(f"score is: {child.getScore()} for child {child.getMove()}")
