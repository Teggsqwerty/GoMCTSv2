...
class game():
    ...
    def initPointer(self):
        for index in range(self.size):
            ...
            lefX,lefY = (x-1),y
            if topY < 0:
                self.blank[index].top = "f"
            else:
                topInd = topX+topY*self.length
                self.blank[index].top = topInd
            if rigX == self.length:
                self.blank[index].right = "f" 
            else:
                rigInd = rigX+rigY*self.length
                self.blank[index].right = rigInd
            if botY == self.length:
                self.blank[index].bottom = "f"
            else:
                botInd = botX+botY*self.length
                self.blank[index].bottom = botInd
            if lefX < 0:
                self.blank[index].left = "f"
            else:
                lefInd = lefX+lefY*self.length
                self.blank[index].left = lefInd
